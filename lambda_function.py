...
GAME_LENGTH = 6
SKILL_NAME = "Raspberry Pi Trivia"
...
QUESTIONS = [
    {"What is the smallest Raspberry Pi called?": ["Raspberry Pi Zero"]},
    {"When was the first Raspberry Pi released?": ["February 2012"]},
    {"What does GPIO stand for?": ["General Purpose Input Output"]},
    {"What is the name of the most recent Raspberry Pi released?": [
        "Raspberry Pi 4"]},
    {"Can you tell me the name of the operating system that is used on the Raspberry Pi?": [
        "Raspbian"]},
    {"In what country was the Raspberry Pi invented?": ["United Kingdom"]},
]
...
def handle_answer_request(intent, session):
    ...
    if 'attributes' in session.keys() and 'questions' not in session['attributes'].keys():
        ...
        if current_questions_index == GAME_LENGTH - 1:
            ...
            spoken_question = QUESTIONS[game_questions[current_questions_index]].keys()[
                0]
            reprompt_text = spoken_question
